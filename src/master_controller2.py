        ...
        if setconfig:
            ...
            for color in self.block_list:
                ...
                if maxxvalue == -100:
                    ...
                    outStr = color + ":" + str(self.block_pose[color].position.x) + "," + str(self.block_pose[color].position.y)
                    f.write(outStr)

            coord = blackoutblock.coord
            self.right_camera.blackout = max(coord[0][0], coord[2][0]) + 15
            outStr = "BLACKOUT:" + str(self.right_camera.blackout)
            f.write(outStr)
            f.close()
        else:
            #Load config file
            ...
            while newLine != "":
                ...
                if len(newLine) == 2:
                    ...
                    if newLine[0] == "BLACKOUT":
                        self.right_camera.blackout = int(newLine[1])
                    if self.box_pose.get(newLine[0],None) != None:
                        coords = newLine[1].split(",")
                        if len(coords) == 2
                            self.box_pose.position.x = coords[0]
                            self.box_pose.position.y = coords[1]
                        else:
